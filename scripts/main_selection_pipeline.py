...
==========================
Complete orchestration of the intelligent song selection process.

This script integrates all components for the complete pipeline:
1. Large dataset analysis (1.2M songs) 
2. Representative selection (10K songs)
...
from scripts.large_dataset_processor import LargeDatasetProcessor
from scripts.representative_selector import RepresentativeSelector
from scripts.selection_validator import SelectionValidator
...
class MainSelectionPipeline:
    """
    Orchestrates the complete song selection pipeline.
    
    Integrates dataset analysis, representative selection, validation,
    and reporting into a seamless automated process.
    """
    ...
    def __init__(self, target_size: int = 10000, output_dir: Optional[str] = None, skip_analysis: bool = False):
        ...
        self.selector = RepresentativeSelector(target_size=target_size, output_dir=str(self.selection_dir))
        ...
        logger.info(f"🎯 Main Selection Pipeline initialized")
    ...
    def stage_2_representative_selection(self, dataset_path: str) -> Optional[str]:
        """
        Stage 2: Select representative songs from the dataset.
        ...
        """
        logger.info("🎯 STAGE 2: Representative Selection")
        logger.info("=" * 60)

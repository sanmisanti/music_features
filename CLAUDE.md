...
- [x] Redaccion LaTeX completa — 7 capitulos (Introduccion, Estado de la Cuestion, Definicion del Problema, Marco Teorico, Solucion Propuesta, Resultados Experimentales, Conclusiones). §4 Marco Teorico (§4.1, §4.2, §4.4, §4.5 con 8 subsecciones, §4.6); §5.7 Sistema de Recomendacion con sus resultados; §1 Introduccion ampliada. El Cap Resultados se elimino el 2026-07-05 (resultados de recomendacion reubicados a §5.7, Costo/Plan de Pruebas descartados) y se **reintrodujo el 2026-07-12** con enfoque cualitativo (demostracion del sistema en funcionamiento): estudios de caso, ablacion por α, probes tematicos y cross-language; Costo Computacional y Plan de Pruebas siguen fuera del informe. Los 3 anexos (A-C) se eliminaron el 2026-06-14 (ver tabla ESTRUCTURA DEL INFORME)
- [ ] Etapa 6: Sintesis — Cap 6 (Resultados Experimentales, cualitativo) redactado 2026-07-12; Anexo A (Interfaz Web) agregado 2026-08-02 con capturas generadas desde codigo; informe compilado sin referencias ni citas indefinidas (111 paginas); pendientes pruebas de calidad externas (ver **[`PLAN_FINALIZACION.md`](PLAN_FINALIZACION.md)** y **[`src/recommendation/PRUEBAS_CALIDAD.md`](src/recommendation/PRUEBAS_CALIDAD.md)**)
...
| Precision@10 hibrido | 0.398 | **0.4447** (α=0.80) | Benchmark de recomendacion; reportado en §5.7.2 |
...
| `thesis/chapters/05_solucion_propuesta.tex` | Cap 5: Solucion Propuesta | Completo (§5.1-§5.7; §5.7 incluye los resultados de recomendacion: grid search de α, metricas al optimo, por genero, ILD y cobertura, reubicados desde el Cap 6 el 2026-07-05) |
| `thesis/chapters/06_resultados.tex` | Cap 6: Resultados Experimentales | **Reintroducido y reescrito 2026-07-12** (revierte la eliminacion del 2026-07-05). Enfoque cualitativo: demostracion del sistema en funcionamiento. 5 secciones: §6.1 Estudios de Caso (α=0.80), §6.2 Ablacion del Peso de Fusion, §6.3 Probes Tematicos, §6.4 Comportamiento entre Idiomas, §6.5 Sintesis. Costo Computacional y Plan de Pruebas NO se reincorporaron. `\input` restaurado en `main.tex` entre §5 y Conclusiones. Tablas generadas desde codigo (ver nota "Cap 6 cualitativo" abajo) |
| `thesis/chapters/07_conclusiones.tex` | Cap 7: Conclusiones y Lineas Futuras | **Reescrito y condensado 2026-08-02**: de 4 secciones (~109 lineas) a 2 secciones de ~1 pagina cada una. §7.1 Conclusiones (que se hizo y logro, aporte de la solucion al problema, alcance/limites en un parrafo, experiencia adquirida) y §7.2 Lineas Futuras (6 desarrollos derivados, de menor a mayor envergadura). Se eliminaron las secciones Contribuciones (tono de paper, contrario al criterio de posicionamiento) y Limitaciones como seccion propia (comprimida a un parrafo de §7.1). Labels eliminados: `sec:contribuciones`, `sec:limitaciones`; las dos `\ref{sec:limitaciones}` externas (§5.7.2 y Anexo A) se redirigieron a `\ref{ch:conclusiones}`. Se conservan `ch:conclusiones`, `sec:conclusiones`, `sec:futuras-lineas`. **Registro exigido (2026-08-02)**: academico y no tecnico, **sin ninguna cifra ni metrica** (ni P@10, ni α, ni dimensiones, ni tamano del catalogo) y sin jerga (nada de k-NN, fusion tardia, Hopkins, NMI, TF-IDF); se narra **como se cumplio el proposito**, no que valores se obtuvieron. Mismo criterio que el Resumen/Abstract de `main.tex`. **Extension y tono**: breve y general (§7.1 en 3 parrafos, ~380 palabras; §7.2 en 6 items de una oracion). NO se detalla el recorrido etapa por etapa (eso ya esta en el Cap 5): el eje es **convencer de que el objetivo se cumplio y el problema quedo resuelto**, apoyandose en las verificaciones del propio sistema (complementariedad de las dimensiones, superioridad de la combinacion, cobertura amplia, reparto de funciones observable caso a caso). Las salvedades se limitan a dos oraciones y se cierran con "ninguna afecta a la conclusion central" |
| `thesis/appendices/A_frontend.tex` | Anexo A: Interfaz Web del Sistema de Recomendacion | **Nuevo 2026-08-02**. Ver nota "Anexo A" abajo |
| `thesis/bibliography.bib` | Referencias (~65 entradas) | En progreso |

**Anexo A: Interfaz Web (2026-08-02)**: primer anexo del informe tras la eliminacion de los tres originales el 2026-06-14. Documenta la aplicacion web (`app/`) en funcionamiento con 8 capturas comentadas. Se inserta en `main.tex` **despues** de `\printbibliography`, precedido de `\appendix` (misma convencion que los anexos originales). **Sin secciones numeradas y con texto minimo** (criterio del usuario 2026-08-02: "es un anexo, no un capitulo"): dos parrafos de contexto (que es la aplicacion, procedencia de las capturas) y una o dos oraciones antes de cada grupo de figuras; la descripcion vive en los pies de figura. Orden de las 8 figuras: estado inicial, busqueda, tablero (\emph{Vuelve}, α=0.80, k=10), ablacion α=0.00, ablacion α=1.00, k=5 y k=20 (\emph{Lose Yourself}), semilla aleatoria (\emph{Rude Boy}). **El anexo es autonomo**: no lleva rutas ni nombres de archivo del proyecto (ni `capture_frontend.py`, ni `results/metrics/`, ni `\texttt{}` alguno) y **no remite a capitulos ni secciones del informe** (cero `\ref`); el analisis interpretativo tampoco se repite, simplemente se omite. Se lee solo, con las imagenes y sus pies. **Capturas generadas desde codigo**: `src/reporting/capture_frontend.py` (Playwright sobre Chrome, ventana 1440×900 con factor de escala 2) produce los 8 PNG en `thesis/figures/app/` y el registro de trazabilidad `results/metrics/etapa6_frontend_captures.json` (semilla, α, k y lista completa de cada figura). Requiere `pip install playwright` y el servidor levantado (`python -m uvicorn app.backend.main:app`). **Observacion util**: el caso de la semilla aleatoria (\emph{Rude Boy} de Rihanna, etiquetada `rap` en el dataset) ilustra en el propio anexo la limitacion del genero como proxy.

**Correcciones al prototipo web derivadas del Anexo A (2026-08-02)**: (1) `app/frontend/styles.css` — se agrego `.board[hidden] { display: none; }`; el `display: grid` de `.board` anulaba el atributo `hidden` del HTML y el esqueleto vacio del tablero era visible antes de elegir semilla. (2) `app/backend/recommender.py` — se porto la deduplicacion de §5.7.3 al prototipo (`RecommenderService.recommend(deduplicate=True)` por defecto), reutilizando `compute_duplicate_groups` y los umbrales de identidad de `engine.py`. La lista servida por la app ya no repite grabaciones; **la evaluacion de la Etapa 5 sigue sin deduplicar** y P@10=0.4447 no se altera (la app no interviene en ese calculo).

**Anexos eliminados (2026-06-14)**: los tres anexos (A Configuraciones, B Metricas, C Reproducibilidad) se eliminaron por ser excesivamente tecnicos para un proyecto de grado. Su contenido util se integro al informe: las 3 tablas exhaustivas de clustering a §5.6 (Solucion Propuesta), la cobertura por genero a §5.7.3 (Metricas Complementarias; inicialmente a §6.1, reubicada el 2026-07-05), y los tiempos de ejecucion a la seccion Costo Computacional (esta ultima integracion quedo sin efecto: el Cap Resultados se elimino el 2026-07-05 y la tabla de tiempos ya no se incluye en el informe). La distribucion de clusters en recomendaciones, el volcado de `config.py` (Anexo A) y la guia de reproducibilidad (Anexo C) se descartaron; el detalle reproducible permanece en el codigo y los JSON de `results/metrics/`.

**Seccion Discusion eliminada (2026-06-18)**: el Cap 6 tenia una §Discusion con tres subsecciones (paradoja Hopkins-Silhouette, dominancia semantica con aporte musical, limitaciones del proxy). Se elimino por redundancia: el tema del proxy/baselines/duplicados/etiquetas ya estaba cubierto, con mas detalle y ejemplos, en §7.3 Limitaciones. Sus dos aportes interpretativos unicos se reubicaron junto a la evidencia que interpretan: (1) el contraste diferencial Hopkins-vs-Silhouette (por que Hopkins detecta estructura bajo concentracion de distancias y Silhouette no; un Silhouette bajo no implica ausencia de estructura) se integro al parrafo de §5.6 donde se introduce el Silhouette 0,0362 frente al Hopkins alto; (2) el mecanismo del rango de fusion (similitudes semanticas comprimidas a [0,69, 1,00] vs kernel musical expandido a [0,00, 1,00]) se integro a §5.7.2 tras la caida de 8,3 puntos de Precision@10 (originalmente en §6.1, reubicado con el resto de los resultados de recomendacion el 2026-07-05). La referencia colgante del analisis por genero (hoy en §5.7.2) se redirigio a §7.3 Limitaciones.

**Resultados de recomendacion reubicados a §5.7 (2026-07-05)**: los resultados del sistema de recomendacion que estaban en §6.1 (Cap 6) se movieron a las subsecciones correspondientes del Cap 5, bajo el criterio de que todo resultado de un mecanismo detallado en el Cap 5 se reporta junto a su desarrollo. Destinos: el grid search de α (cuadro `recommendation_grid_search` + figura `recommendation_alpha_precision`) y el desglose Precision@10 por genero (`recommendation_precision_per_genre` + figura `recommendation_precision_genre`) a §5.7.2 (Optimizacion de Pesos), en el bloque "Resultados de la optimizacion"; el scorecard consolidado (`recommendation_optimal_metrics`) y la cobertura por genero (`coverage_per_genre`) a §5.7.3 (Metricas Complementarias). Se elimino la duplicacion de ILD/cobertura que existia entre §5.7.3 y §6.1, corrigiendo de paso una incoherencia numerica de la cobertura (17.156 erroneo en §6.1 vs 17.162 correcto en §5.7.3; queda solo 17.162). Poco despues, el mismo 2026-07-05, el Cap Resultados se elimino por completo del informe: las secciones remanentes (Costo Computacional y Plan de Pruebas de Calidad) se descartaron y el `\input{chapters/06_resultados}` se quito de `main.tex`. Con ello, Conclusiones pasa a ser el Cap 6 en el PDF (su archivo conserva el nombre `07_conclusiones.tex`).

**Cap 6 cualitativo reintroducido (2026-07-12)**: se reintrodujo el Cap 6 (`06_resultados.tex`, titulo conservado "Resultados Experimentales"), esta vez con un **enfoque exclusivamente cualitativo** que demuestra el sistema en funcionamiento (recomendaciones reales para canciones concretas, verificando su coherencia). Costo Computacional y Plan de Pruebas **no** se reincorporaron (el plan queda solo como guia interna en `src/recommendation/PRUEBAS_CALIDAD.md`). El `\input{chapters/06_resultados}` se restauro en `main.tex` entre §5 y Conclusiones; Conclusiones vuelve a ser Cap 7 y la `\ref{ch:resultados}` de su linea 12 (que habia quedado colgada) vuelve a resolver. Estructura: §6.1 Estudios de Caso, §6.2 Ablacion del Peso de Fusion (α∈{0,0.80,1.0}), §6.3 Probes Tematicos, §6.4 Comportamiento entre Idiomas, §6.5 Sintesis. **Hallazgos clave** (honestos, alineados con el tono anti-sobreafirmacion): (1) caso insignia \emph{Vuelve} recupera baladas de perdida en español e ingles = evidencia de fusion multimodal; (2) la ablacion muestra que la modalidad musical aporta proximidad acustica agnostica al idioma y la semantica fija idioma+registro; (3) gradiente de captura tematica: temas de firma lexica concreta (fiesta) bien capturados, temas de registro afectivo (duelo, \emph{See You Again}) parcialmente, temas abstractos/ideologicos (protesta) solo marginalmente; (4) el espacio semantico **agrupa por idioma** — el cruce entre idiomas lo aporta la modalidad musical, no la comprension multilingüe (templa la premisa del E5 multilingüe). **Codigo nuevo** (tablas y figuras desde codigo, no a mano): `src/recommendation/generate_qualitative.py` (genera datos → `results/qualitative_report.txt` + `results/metrics/etapa6_qualitative.json`) y `src/recommendation/qualitative_tables.py` (8 tablas LaTeX: `qual_case_vuelve`, `qual_case_lose_yourself`, `qual_ablation_vuelve`, `qual_ablation_lose_yourself`, `qual_ablation_shape_of_you`, `qual_probe_celebracion`, `qual_probe_protesta`, `qual_crosslanguage`). Ambos reproducibles (seed fijo). `pipeline_timing.tex` queda huerfano (ya no se incluye).

**Deduplicacion de recomendaciones (decision 2026-07-12)**: el dataset fuente contiene la misma grabacion con `track_id` distintos (recolectada desde playlists diferentes; 1.901 entradas duplicadas sobre 17.964, 16.063 grabaciones unicas). **Justificacion adoptada**: los duplicados son naturales en catalogos musicales (una cancion figura en varios albumes, ediciones o recopilatorios), y que el sistema los detecte como lo mas similar **valida** que la medida de similitud funciona (una copia exacta es lo mas parecido posible). Por eso: (a) la **evaluacion cuantitativa** (Etapa 5, §5.7, P@10=0.4447) se conserva **sin deduplicar** — no se re-ejecuto `run_recommendation.py`; (b) solo se deduplica **la salida mostrada al usuario / los ejemplos del Cap 6**, porque no se recomienda dos veces la misma cancion. Implementacion: dedup en post-procesamiento en `generate_qualitative.py` (y como opcion `dup_groups` en `engine.recommend_all`, **no usada** por la evaluacion). Reglas: colapsar por (titulo, artista) normalizados via `compute_duplicate_groups` + `normalize_text` en `engine.py`; excluir la propia grabacion de la consulta; y capturar grabaciones identicas con metadatos distintos (mismo vector: sem≥0.9999 y mus≥0.999) que el nombre no agrupa. **Se conservan** variantes legitimas (remaster, vivo, cover, traduccion), que difieren en el vector musical (ej.: "Vuelve - MTV Unplugged" mus=0.612 se mantiene; "Lose Yourself - From 8 Mile" mus=0.9998 se colapsa). `optimizer.py` y `run_recommendation.py` quedaron **revertidos** (sin dedup) para que §5.7 reproduzca 0.4447 exacto. **Documentacion en el informe**: se agrego la subseccion §5.7.3 "Deduplicacion de la Lista de Recomendacion" en `05_solucion_propuesta.tex` (justifica por que no se limpia el dataset ni la evaluacion, y como se depura la lista servida; label `subsec:deduplicacion`), con lo que "Metricas Complementarias" paso a §5.7.4. El Cap 6 (caso Lose Yourself) lo ilustra y remite a esa subseccion.

Tablas generadas desde codigo (no se editan a mano):
- **Cobertura por genero** (`tab:coverage_per_genre`): `generate_genre_coverage_table()` en `src/recommendation/tables.py`, invocada desde `run_recommendation.py`. Lee `genre_coverage` de las metricas de Etapa 5. Se incluye en §5.7.3.
- **Tiempos de ejecucion** (`tab:tiempos_ejecucion`): **ya no se incluye en el informe** (la seccion Costo Computacional se elimino con el Cap Resultados el 2026-07-05; el generador se conserva por si se decide reintroducir la tabla). `generate_timing_table()` en **`src/reporting/timing.py`** (`python -m src.reporting.timing`). Agrega el tiempo total de cada etapa leyendo `results/metrics/`. Requiere instrumentacion: cada `run_*.py` ahora registra `total_elapsed_seconds` de nivel superior en su JSON. Hasta que cada etapa se re-ejecute con esta instrumentacion, el generador usa valores de respaldo de logs de la ejecucion previa (solo vectorizacion y recomendacion ya tienen su tiempo persistido en JSON). Re-ejecutar las etapas baratas (segundos) y clustering (~40 min) hace que la tabla provenga unicamente de los JSON; la vectorizacion (2h) no necesita repetirse.
